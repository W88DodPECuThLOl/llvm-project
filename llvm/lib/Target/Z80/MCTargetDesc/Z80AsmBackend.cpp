...
public:
  Z80AsmBackend(uint8_t OSABI)
      : MCAsmBackend(support::little), OSABI(OSABI) {}
  ~Z80AsmBackend() override {}
  ...
  void relaxInstruction(MCInst &Inst,
    ...
    report_fatal_error("Z80AsmBackend::relaxInstruction() unimplemented");
  }

  virtual bool writeNopData(raw_ostream &OS, uint64_t Count, const MCSubtargetInfo *STI) const override;
};

bool Z80AsmBackend::writeNopData(raw_ostream &OS, uint64_t Count, const MCSubtargetInfo *STI) const {
  if ((Count % 4) != 0)
    return false;
...
Z80AsmBackend::createObjectTargetWriter() const {
  return createZ80ELFObjectWriter(OSABI);
}
...
MCAsmBackend *llvm::createZ80AsmBackend(const Target &T,
  ...
  return new Z80AsmBackend(OSABI);
}
