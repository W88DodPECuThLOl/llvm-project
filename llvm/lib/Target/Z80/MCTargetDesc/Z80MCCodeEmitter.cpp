...
public:
  Z80MCCodeEmitter(MCContext &ctx, const MCInstrInfo &MCII)
      : Ctx(ctx), MCII(MCII) {}
...
MCCodeEmitter *llvm::createZ80MCCodeEmitter(const MCInstrInfo &MCII,
                                               MCContext &Ctx) {
  return new Z80MCCodeEmitter(Ctx, MCII);
}
...
unsigned
Z80MCCodeEmitter::getMachineOpValue(const MCInst &MI, const MCOperand &MO,
                                       SmallVectorImpl<MCFixup> &Fixups,
                                       const MCSubtargetInfo &STI) const {
