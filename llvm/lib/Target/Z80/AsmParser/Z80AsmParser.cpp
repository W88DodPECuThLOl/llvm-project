...
class Z80AsmParser : public MCTargetAsmParser {
  ...
  virtual OperandMatchResultTy tryParseRegister(MCRegister &Reg, SMLoc &StartLoc, SMLoc &EndLoc) override;
  virtual bool parseRegister(MCRegister &Reg, SMLoc &StartLoc, SMLoc &EndLoc) override;
...
/// Z80Operand - Instances of this class represent a parsed machine
/// instruction
...
public:
  Z80Operand(const Z80Operand &o) : MCParsedAsmOperand() {
    Kind = o.Kind;
    StartLoc = o.StartLoc;
    EndLoc = o.EndLoc;
    switch (Kind) {
  ...
  static std::unique_ptr<Z80Operand> createToken(StringRef Str, SMLoc S) {
    auto Op = std::make_unique<Z80Operand>(Token);
    Op->Tok = Str;
    Op->StartLoc = S;
    Op->EndLoc = S;
    return Op;
  ...
  static std::unique_ptr<Z80Operand> createReg(unsigned RegNo, SMLoc S,
                                                  SMLoc E) {
    auto Op = std::make_unique<Z80Operand>(Register);
    Op->Reg.RegNum = RegNo;
    Op->StartLoc = S;
    Op->EndLoc = E;
    return Op;
  ...
  static std::unique_ptr<Z80Operand> createImm(const MCExpr *Val, SMLoc S,
                                                  SMLoc E) {
    auto Op = std::make_unique<Z80Operand>(Immediate);
    Op->Imm.Val = Val;
    Op->StartLoc = S;
    Op->EndLoc = E;
    return Op;
...
bool Z80AsmParser::generateImmOutOfRangeError(
    OperandVector &Operands, uint64_t ErrorInfo, int64_t Lower, int64_t Upper,
    Twine Msg = "immediate must be an integer in the range") {
  SMLoc ErrorLoc = ((Z80Operand &)*Operands[ErrorInfo]).getStartLoc();
...
bool Z80AsmParser::MatchAndEmitInstruction(SMLoc IDLoc, unsigned &Opcode,
  ...
  case Match_InvalidOperand: {
    ...
    if (ErrorInfo != ~0U) {
      ...
      ErrorLoc = ((Z80Operand &)*Operands[ErrorInfo]).getStartLoc();
      if (ErrorLoc == SMLoc())
        ErrorLoc = IDLoc;
    }
  ...
  Z80Operand &Operand = (Z80Operand &)*Operands[ErrorInfo];
  if (!Operand.isImm())
...
OperandMatchResultTy Z80AsmParser::tryParseRegister(MCRegister &Reg,
                                                       SMLoc &StartLoc,
                                                       SMLoc &EndLoc) {
...
OperandMatchResultTy Z80AsmParser::parseRegister(OperandVector &Operands) {
  SMLoc S = getLoc();
...
OperandMatchResultTy Z80AsmParser::parseImmediate(OperandVector &Operands) {
  switch (getLexer().getKind()) {
  default:
    return MatchOperand_NoMatch;
  ...
  SMLoc E = SMLoc::getFromPointer(S.getPointer() - 1);
  Operands.push_back(Z80Operand::createImm(IdVal, S, E));
  return MatchOperand_Success;
}
...
bool Z80AsmParser::ParseDirective(AsmToken DirectiveID) { return true; }
